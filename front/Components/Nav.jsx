    ...
    <>
      <nav
        ...
        className={`fixed w-full top-0 left-0 right-0 z-[100] transition-all duration-700 font-heading ${
          scrolled
            ? 'py-3 backdrop-blur-[40px] bg-background/80 border-b border-border shadow-lg'
            : 'py-6 bg-gradient-to-b from-background/90 via-background/30 to-transparent'
        }`}
      >
          ...
          <div className="flex items-center justify-between w-full relative">
            ...
            <Link href="/" className="flex items-center gap-4 group relative z-10">
              ...
              <div className="hidden lg:flex flex-col relative justify-center">
                <span className="text-2xl md:text-3xl font-black leading-none tracking-tighter bg-clip-text text-transparent bg-gradient-to-br from-foreground via-foreground to-foreground/50 group-hover:from-primary group-hover:to-red-500 transition-all">
                  ZAMALEK SC
                </span>
                <div className="flex items-center gap-2 mt-1">
                  <span className="w-4 h-px bg-primary" />
                  <span className="text-[10px] md:text-xs font-black uppercase tracking-[0.4em] text-foreground/50">
                    The White Knights
                  </span>
                </div>
              </div>
                  ...
                  >
                    ...
                    >
                      {activeDropdown === link.id && (
                        <motion.div
                          layoutId="navPill"
                          className="absolute inset-0 bg-foreground/10 backdrop-blur-md border border-foreground/20 rounded-full"
                    ...
                    <AnimatePresence>
                      {link.sublinks && activeDropdown === link.id && (
                        <motion.div
                          ...
                          className="absolute top-full right-1/2 translate-x-1/2 mt-6 w-[500px] bg-background/90 backdrop-blur-3xl border border-foreground/10 rounded-3xl shadow-[0_20px_60px_-15px_rgba(227,27,35,0.3)] overflow-hidden"
                        >
                          <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-l from-primary via-red-600 to-background" />
                            ...
                            {link.sublinks.map((sublink, index) => {
                              ...
                              return (
                                ...
                                >
                                  <Link
                                    href={sublink.url}
                                    className="flex items-start gap-4 p-4 rounded-2xl hover:bg-foreground/5 border border-transparent hover:border-foreground/10 transition-all group/item"
                                  >
                                    <div className="w-10 h-10 shrink-0 rounded-xl bg-foreground/5 flex items-center justify-center group-hover/item:bg-primary group-hover/item:scale-110 transition-all shadow-inner">
                                      {SubIcon && <SubIcon size={18} className="text-foreground/70 group-hover/item:text-foreground" />}
                                    </div>
                                    <div className="flex flex-col">
                                      <span className="text-base font-bold text-foreground mb-1 group-hover/item:text-primary transition-colors">{sublink.title}</span>
                                      <span className="text-xs text-foreground/40 font-semibold leading-relaxed">{sublink.desc}</span>
                                    </div>
                                  </Link>
                                </motion.div>
                              );
            ...
            <div className="flex items-center gap-3 z-10">
              ...
              <Link
                href="/Pages/Store"
                className="hidden sm:flex items-center gap-2 px-6 py-2.5 rounded-full bg-foreground/10 hover:bg-primary text-foreground border border-foreground/20 hover:border-primary text-sm font-black tracking-wide transition-all hover:scale-105 group"
              >
              ...
              <motion.button
                ...
                className="p-3 rounded-full bg-foreground/5 border border-foreground/10 hover:bg-foreground/10 transition-colors text-foreground"
                aria-label="Search"
              >
                <Search size={18} />
              </motion.button>
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={toggleTheme}
                className="p-3 rounded-full bg-foreground/5 border border-foreground/10 hover:bg-foreground/10 transition-colors relative overflow-hidden"
                aria-label="Theme"
              >
                <AnimatePresence mode="wait">
                  {theme === 'dark' ? (
                    ...
                    <motion.div key="moon" initial={{ rotate: 90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: -90, opacity: 0 }}>
                      <Moon size={18} className="text-foreground drop-shadow-[0_0_5px_rgba(255,255,255,0.8)]" />
                    </motion.div>
                  )}
                </AnimatePresence>
              </motion.button>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {user?.isAdmin && (
                    <motion.button
                      ...
                      className="p-3 text-red-400 hover:text-foreground hover:bg-primary rounded-full transition-all"
                    >
                      <MdEdit size={18} />
                    </motion.button>
                  )}
        ...
        {isOpen && (
          <>
            <motion.div
              ...
              className="fixed inset-0 z-[105] bg-background/80 backdrop-blur-md lg:hidden"
            />
            <motion.div
              ...
              className="fixed inset-y-0 right-0 z-[110] w-full max-w-[85%] sm:max-w-md bg-background border-l border-foreground/10 shadow-[0_0_80px_rgba(227,27,35,0.15)] lg:hidden overflow-hidden flex flex-col"
              dir="rtl"
            >
              {/* Header */}
              <div className="p-6 flex items-center justify-between border-b border-foreground/5 bg-foreground/5">
                <Link href="/" onClick={() => setIsOpen(false)} className="flex items-center gap-3">
                  <div className="relative w-10 h-10">
                    <Image src="/zsc.png" alt="ZSC" fill className="object-contain" />
                  </div>
                  <div>
                    <div className="text-xl font-black text-foreground">ZAMALEK SC</div>
                    <div className="text-[9px] uppercase tracking-widest text-primary">The Royal Club</div>
                  </div>
                </Link>
                <button onClick={() => setIsOpen(false)} className="p-2 rounded-full bg-foreground/10 text-foreground hover:bg-primary transition-all">
                  <X size={20} />
                </button>
              </div>
                ...
                <div className="space-y-2">
                  {links.map((link, idx) => (
                    <div key={link.id}>
                      {link.sublinks ? (
                        <div className="mb-4">
                          <div className="text-[10px] font-black tracking-widest text-foreground/30 uppercase mb-3 flex items-center gap-2">
                          ...
                          <div className="space-y-1">
                            {link.sublinks.map(sub => (
                              <Link
                                ...
                                href={sub.url}
                                onClick={() => setIsOpen(false)}
                                className="flex items-center gap-4 p-3 rounded-2xl hover:bg-foreground/5 transition-all text-foreground/80 hover:text-foreground group"
                              >
                                <div className="w-8 h-8 rounded-full bg-foreground/5 flex items-center justify-center group-hover:bg-primary group-hover:text-foreground transition-all text-foreground/50">
                        ...
                        <Link
                          href={link.url}
                          onClick={() => setIsOpen(false)}
                          className="flex items-center justify-between p-4 rounded-2xl bg-foreground/5 border border-foreground/5 hover:border-primary text-foreground font-black text-lg group transition-all mb-2"
                        >
              ...
              {/* Mobile Footer CTAs */}
              <div className="p-6 border-t border-foreground/5 bg-gradient-to-t from-primary/10 to-transparent">
                <Link
                  href="/Pages/Fixtures"
                  onClick={() => setIsOpen(false)}
                  className="w-full flex items-center gap-3 justify-center py-4 rounded-2xl bg-foreground text-background font-black text-lg mb-3 hover:scale-[1.02] transition-all"
                >
        ...
        {searchOpen && (
          <>
            ...
            <motion.div
              ...
              dir="rtl"
            >
              <div className="bg-card border border-foreground/10 rounded-[2rem] shadow-[0_20px_80px_-20px_rgba(227,27,35,0.6)] p-2 relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-primary to-transparent" />
                <div className="flex items-center gap-4 bg-foreground/5 rounded-full p-2 pl-6">
                  ...
                  <input
                    ...
                    className="flex-1 bg-transparent text-xl font-bold text-foreground outline-none placeholder:text-foreground/30"
                    autoFocus
                  />
                  <div className="text-[10px] text-foreground/30 font-bold tracking-widest hidden sm:block">ESC للإغلاق</div>
                  <button onClick={() => setSearchOpen(false)} className="sm:hidden p-3 text-foreground/50 hover:text-foreground bg-foreground/10 rounded-full">
                    <X size={20} />
                  </button>
                </div>
              </div>
